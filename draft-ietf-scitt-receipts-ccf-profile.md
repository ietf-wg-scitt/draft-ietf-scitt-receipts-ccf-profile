...
A CCF consistency proof establishes that the tree of `m` transactions with root `R_m` is a prefix of the tree of `n > m` transactions with root `R_n`: every transaction of the older tree is at the same position in the newer tree.

~~~ cddl
ccf-consistency-proof = bstr .cbor {
  ; Root of the rightmost complete subtree of the older tree
...
The payload is the newer root `R_n`, and MUST be detached. The older root `R_m` is not carried by the receipt: the verifier already holds it, typically as the root recomputed from an inclusion receipt ({{ccf-inclusion-receipt-verification}}), and checks that the proof recomputes it.
...
verify_consistency_receipt(consistency_receipt, older_root):
  ...
  for proof in vdp[CONSISTENCY_PROOF_LABEL]:
      let older, newer = compute_roots(proof)
      if older == older_root:
          # Use the newer Merkle Root as the detached payload
          return verify_cose(consistency_receipt, newer)
  return false
~~~
{: #ccf-consistency-receipt-verification title="CCF Consistency Receipt Verification"}

`CONSISTENCY_PROOF_LABEL` is the `consistency-proof` proof type label (-2) defined by {{-cose-receipts}}. The comparison with `older_root` binds the receipt to a state the verifier already trusts and validates the anchor: a receipt whose recomputed older root differs from `older_root` proves nothing about that state.
...
At least one of `inclusion-proof` and `consistency-proof` MUST be present. All proofs in a receipt recompute the same root, which is the detached payload. The proof type is conveyed by the key of the `vdp` map, as specified by {{-cose-receipts}}; it is not carried in a separate header parameter.
